          ...
          mWatchdog(-1), mLastComplete(0), mCmdNum(0)
{
    ...
    if (mPid == 0) {
        std::cout << "Exiftool: create exiftool thread " << getpid() << std::endl;
        close(to[1]);
        close(from[0]);
        close(err[0]);
        ...
        std::cout << "Exiftool: encountered working thread " << getpid() <<std::endl;
        close(to[0]);
        close(from[1]);
        close(err[1]);
        ...
        if (mPid != -1 && !sNoWatchdog) {
            ...
            if (!mWatchdog) {
                ...
                std::cout << "Exiftool: created watchdog thread " << getpid() << " to look for presents of " << pid << std::endl;
                for (;;) {
                    sleep(10);
